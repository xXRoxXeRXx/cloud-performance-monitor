...
| **ServiceTestFailure** | Test-Fehler oder keine erfolgreichen Tests in 20min | `cloud_test_success{error_code!="none"} == 0` oder `absent_over_time(cloud_test_success{error_code="none"}[20m])` | 30m | Admin, DevOps |
...
| **RepeatedServiceFailures** | Mehr als 5 Fehler in 30min | `increase(cloud_test_errors_total[30m]) > 5` | 2m | DevOps |
| **HighErrorRate** | Fehlerrate über 10% in 30min | `rate(cloud_test_errors_total[30m]) > 0.1` | 2m | DevOps |
...
| **RepeatedServiceUnavailableErrors** | Wiederholte 503-Fehler (>2 in 45min) | `increase(cloud_test_errors_total{error_type="http_503_unavailable"}[45m]) > 2` | 5m | DevOps |
| **DownloadIncompleteError** | Download unvollständig | `cloud_test_success{error_code="download_failed"} == 0` | 1m | DevOps |
| **RepeatedConflictErrors** | Wiederholte 409-Fehler (>3 in 45min) | `increase(cloud_test_errors_total{error_type="http_409_conflict"}[45m]) > 3` | 2m | DevOps |
| **ClientTimeoutErrors** | Client-Timeout-Fehler | `cloud_test_success{error_code="network_timeout"} == 0` | 1m | Network Team |
| **RepeatedTimeoutErrors** | Wiederholte Timeout-Fehler (>2 in 45min) | `increase(cloud_test_errors_total{error_type="network_timeout"}[45m]) > 2` | 5m | Network Team |
...
- **Kritische Fehlerrate**: > 50% in 30 Minuten
- **Hohe Fehlerrate**: > 10% in 30 Minuten
- **Connection Timeouts**: > 5% in 10 Minuten

### Wiederholungsschwellwerte (angepasst für 15-min Test-Intervall)
- **Service-Fehler**: > 5 in 30 Minuten
- **503-Fehler**: > 2 in 45 Minuten
- **409-Konflikte**: > 3 in 45 Minuten
...
- **Erste Benachrichtigung**: 10 Minuten Gruppierung
- **Wiederholung**: Alle 4 Stunden
- **Zielgruppe**: DevOps

### 🌐 **Netzwerk Alerts** (category: network)
- **Erste Benachrichtigung**: 15 Minuten Gruppierung
...
**WICHTIG**: Alle Wiederholungsalarme wurden an das 15-Minuten-Test-Intervall angepasst:

- **Minimum Zeitfenster**: 30-45 Minuten (mindestens 2-3 Test-Zyklen)
- **Logik**: Bei Tests alle 15 Minuten sind Alarme mit Zeitfenstern < 20 Minuten sinnlos
- **Beispiel**: Ein "Wiederholungs"-Alarm über 15 Minuten kann maximal 1 Test erfassen

### Angepasste Zeitfenster:
- **Error Rates**: 5min → **30min** (für sinnvolle Ratenberechnung)
- **Repeated Failures**: 15min → **30min** (RepeatedServiceFailures)
- **503 Errors**: 15min → **45min** (RepeatedServiceUnavailableErrors)  
- **409 Conflicts**: 15min → **45min** (RepeatedConflictErrors)
- **Timeouts**: 20min → **45min** (RepeatedTimeoutErrors)
- **Directory Creation**: 10min → **30min** (DirectoryCreationFailed)
