...
func (c *Client) UploadFile(filePath string, reader io.Reader, size int64, chunkSize int64) error {
	...
	log.Printf("[Nextcloud] Starting chunked upload for %s (size: %d bytes, chunk size: %d bytes, transfer ID: %s)", filePath, size, chunkSize, transferID)

	// 1. Create temporary directory for chunks on the server
	log.Printf("[Nextcloud] Creating chunk directory: %s", chunkDir)
	mkcolStart := time.Now()
	
	req, err := http.NewRequest("MKCOL", chunkDirURL, nil)
	if err != nil {
		log.Printf("[Nextcloud] ERROR: Could not create MKCOL request: %v", err)
	...
	if err != nil {
		log.Printf("[Nextcloud] ERROR: MKCOL request failed after %v: %v", mkcolDuration, err)
	...
	if resp.StatusCode != http.StatusCreated {
		...
		log.Printf("[Nextcloud] ERROR: MKCOL failed with status %s after %v, response: %s", resp.Status, mkcolDuration, string(body))
		return fmt.Errorf("MKCOL for chunks failed with status %s after %v", resp.Status, mkcolDuration)
	}
	
	log.Printf("[Nextcloud] SUCCESS: Chunk directory created in %v (status: %s)", mkcolDuration, resp.Status)

	// 2. Upload file in chunks
	log.Printf("[Nextcloud] Starting chunk upload phase...")
	if err := c.uploadChunks(chunkDir, reader, chunkSize, destinationURL); err != nil {
		log.Printf("[Nextcloud] ERROR: Chunk upload failed: %v", err)
		return err
	}
	log.Printf("[Nextcloud] All chunks uploaded successfully")

	// 3. Assemble chunks by moving the directory
	moveSource := c.BaseURL + chunkDir + "/.file"
	
	log.Printf("[Nextcloud] Starting MOVE operation from %s to %s", moveSource, destinationURL)
	...
	log.Printf("[Nextcloud] Executing MOVE operation (this may take several minutes for large files)...")
	moveStart := time.Now()
	resp, err = moveClient.Do(req)
	moveDuration := time.Since(moveStart)
	
	if err != nil {
		log.Printf("[Nextcloud] MOVE operation failed after %v: %v", moveDuration, err)
		return fmt.Errorf("MOVE request failed after %v: %w", moveDuration, err)
	}
	defer resp.Body.Close()

	log.Printf("[Nextcloud] MOVE operation completed in %v with status %s", moveDuration, resp.Status)

	if resp.StatusCode != http.StatusCreated && resp.StatusCode != http.StatusNoContent {
		// Read response body for more detailed error information
		body, _ := io.ReadAll(resp.Body)
		log.Printf("[Nextcloud] MOVE failed with status %s, response: %s", resp.Status, string(body))
...
func (c *Client) uploadChunks(chunkDir string, reader io.Reader, chunkSize int64, destinationURL string) error {
	...
	log.Printf("[Nextcloud] Starting chunk upload to %s (chunk size: %d bytes)", chunkDir, chunkSize)
	...
	for {
		...
		if bytesRead > 0 {
			...
			log.Printf("[Nextcloud] Uploading chunk %d: %d bytes to %s", chunkNumber, bytesRead, chunkPath)
			chunkStart := time.Now()
			...
			for attempt := 1; attempt <= maxRetries; attempt++ {
				...
				if err != nil {
					log.Printf("[Nextcloud] ERROR: Could not create PUT request for chunk %d (attempt %d): %v", chunkNumber, attempt, err)
				...
				if chunkErr != nil {
					log.Printf("[Nextcloud] WARNING: PUT request for chunk %d failed (attempt %d/%d) after %v: %v", chunkNumber, attempt, maxRetries, time.Since(chunkStart), chunkErr)
					if attempt < maxRetries {
				...
				if resp.StatusCode == http.StatusCreated || resp.StatusCode == http.StatusNoContent || resp.StatusCode == http.StatusOK {
					...
					body, _ := io.ReadAll(resp.Body)
					resp.Body.Close()
					log.Printf("[Nextcloud] WARNING: Chunk %d upload failed (attempt %d/%d) with status %s after %v, response: %s", chunkNumber, attempt, maxRetries, resp.Status, time.Since(chunkStart), string(body))
					if attempt < maxRetries {
						time.Sleep(time.Duration(attempt) * time.Second) // Progressive backoff
						continue
					}
			...
			log.Printf("[Nextcloud] SUCCESS: Chunk %d uploaded successfully in %v (status: %s)", chunkNumber, chunkDuration, resp.Status)
		...
		if readErr != nil {
			log.Printf("[Nextcloud] ERROR: Failed to read chunk %d: %v", chunkNumber, readErr)
			return fmt.Errorf("failed to read chunk %d: %w", chunkNumber, readErr)
		}
	}
	
	log.Printf("[Nextcloud] Chunk upload summary: %d/%d chunks uploaded successfully", successfulChunks, totalChunks)
	return nil
}
