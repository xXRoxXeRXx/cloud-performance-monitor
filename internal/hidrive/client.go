...
func (c *Client) EnsureDirectory(dirPath string) error {
	...
	resp, err := httpRetry.DoWithRetryAndLog(req.Context(), c.HTTPClient, req, "ensure_directory", "hidrive", c.BaseURL)
	if err != nil {
		return err
	}
...
func (c *Client) UploadFile(filePath string, reader io.Reader, size int64, chunkSize int64) error {
	...
	resp, err := httpRetry.DoWithRetryAndLog(req.Context(), c.HTTPClient, req, "create_chunk_directory", "hidrive", c.BaseURL)
	...
	resp, err = moveHttpRetry.DoWithRetryAndLog(req.Context(), moveClient, req, "move_chunks", "hidrive", c.BaseURL)
	...
	for {
		...
		if bytesRead > 0 {
			...
			for attempt := 1; attempt <= maxRetries; attempt++ {
				...
				if chunkErr != nil {
					c.logger.LogOperation(utils.ERROR, "hidrive", c.BaseURL, "chunk_upload", "http_error", 
				...
				if resp.StatusCode == http.StatusCreated || resp.StatusCode == http.StatusNoContent || resp.StatusCode == http.StatusOK {
					...
					body, _ := io.ReadAll(resp.Body)
					resp.Body.Close()
					c.logger.LogOperation(utils.ERROR, "hidrive", c.BaseURL, "chunk_upload", "status_error", 
...
func (c *Client) DownloadFile(filePath string) (io.ReadCloser, error) {
	...
	resp, err := httpRetry.DoWithRetryAndLog(req.Context(), c.HTTPClient, req, "download_file", "hidrive", c.BaseURL)
	if err != nil {
		return nil, err
	}
...
func (c *Client) DeleteFile(filePath string) error {
	...
	resp, err := httpRetry.DoWithRetryAndLog(req.Context(), c.HTTPClient, req, "delete_file", "hidrive", c.BaseURL)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
